...
public class GameManager : MonoBehaviour
{
    ...
    public void GameOver()
    {
        ...
        if (isGameOverHandled)
            return; // Đã xử lý rồi thì không làm lại nữa

        isGameOverHandled = true;

        // Add other game over actions here if necessary
        int bonus = ScoreManager.Instance.Score + 2 * PlayerDataManager.Instance.vipNFT;

        // Lấy điểm cũ từ PlayerPrefs (mặc định 0 nếu chưa có)
        int oldScore = PlayerPrefs.GetInt("TotalScore", 0);

        Debug.Log("Game Over");

        // Cộng và lưu lại
        int newScore = oldScore + bonus;
        Debug.Log("Game Over" + newScore);
        PlayerPrefs.SetInt("TotalScore", newScore);
        PlayerPrefs.Save();  // Lưu ngay để tránh mất dữ liệu khi đóng game
    }
